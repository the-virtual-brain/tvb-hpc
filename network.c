...
#ifdef CACHING_SHARED_SYNC/*{{{*/
    __shared__ float shr_weights[WARP_SIZE];
    __shared__ unsigned int shr_idelay[WARP_SIZE];
#endif // CACHING_SHARED_SYNC/*}}}*/

#ifdef CACHING_SHARED/*{{{*/
    __shared__ float shr_buffer[2 * BLOCK_DIM_X];
    float *shr_weights = shr_buffer;
    unsigned int *shr_idelay = (unsigned int *) shr_buffer + blockDim.x;
#endif // CACHING_SHARED/*}}}*/
...
#ifdef CACHING_SHUFFLE/*{{{*/
            for (unsigned int j_node = 0; j_node < n_node;)
            {
                const float wiT = (j_node+threadIdx.x%WARP_SIZE<n_node) ?  weights[i_n + j_node + threadIdx.x%WARP_SIZE] : 0.;
                const unsigned int diT = (j_node+threadIdx.x%WARP_SIZE<n_node) ? lengths[i_n + j_node + threadIdx.x%WARP_SIZE] * rec_speed_dt : 0;
                do
                {
                    //float wij = weights[i_n + j_node]; // nb. not coalesced
                    const float wij = __shfl(wiT, j_node%WARP_SIZE);
                    if (wij == 0.0)
                    {
                        ++j_node;
                        continue;
                    ...
                    const unsigned int dij = __shfl(diT, j_node%WARP_SIZE);
                    const float theta_j = state((t - dij + NH) % NH, j_node);
                    sum += wij * sin(theta_j - theta_i);

                    ++j_node;
                } while((j_node%WARP_SIZE) && j_node<n_node);
            } // j_node
/*}}}*/
#elif defined(CACHING_SHARED)/*{{{*/

            for (unsigned int j_node = 0; j_node < n_node;)
            {
                shr_weights[threadIdx.x] = (j_node+threadIdx.x%WARP_SIZE<n_node) ?  weights[i_n + j_node + threadIdx.x%WARP_SIZE] : 0.;
                shr_idelay[threadIdx.x] = (j_node+threadIdx.x%WARP_SIZE<n_node) ? lengths[i_n + j_node + threadIdx.x%WARP_SIZE] * rec_speed_dt : 0;

                do
                {
                    //float wij = weights[i_n + j_node]; // nb. not coalesced
                    const float wij = shr_weights[j_node%WARP_SIZE + threadIdx.x/WARP_SIZE*WARP_SIZE];
                    if (wij == 0.0)
                    {
                        ++j_node;
                        continue;
                    ...
                    //unsigned int dij = lengths[i_n + j_node] * rec_speed_dt;
                    const unsigned int dij = shr_idelay[j_node%WARP_SIZE + threadIdx.x/WARP_SIZE*WARP_SIZE];
                    const float theta_j = state((t - dij + NH) % NH, j_node);
                    sum += wij * sin(theta_j - theta_i);
...
/*}}}*/
#elif defined(CACHING_SHARED_SYNC)/*{{{*/

            for (unsigned int j_node = 0; j_node < n_node;)
            {
                if (threadIdx.x < WARP_SIZE)
                {
                    shr_weights[threadIdx.x] = (j_node+threadIdx.x<n_node) ?  weights[i_n + j_node + threadIdx.x] : 0.;
                    shr_idelay[threadIdx.x] = (j_node+threadIdx.x<n_node) ? lengths[i_n + j_node + threadIdx.x] * rec_speed_dt : 0;
                }
                __syncthreads();
                ...
                {
                    //float wij = weights[i_n + j_node]; // nb. not coalesced
                    const float wij = shr_weights[j_node%WARP_SIZE];
                    if (wij == 0.0)
                    {
                        ++j_node;
                        continue;
                    ...
                    //unsigned int dij = lengths[i_n + j_node] * rec_speed_dt;
                    const unsigned int dij = shr_idelay[j_node%WARP_SIZE];
                    const float theta_j = state((t - dij + NH) % NH, j_node);
                    sum += wij * sin(theta_j - theta_i);
...
#ifdef RAND123/*{{{*/
            theta_i += sig * rng_next_normal(&rng); // noise(t - i_step, i_node);
#endif
#ifdef CURAND
